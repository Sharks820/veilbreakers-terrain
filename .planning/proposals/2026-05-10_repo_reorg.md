...
| `assets/free/{hdris,pbr,scatter,trees,water}/` | 2.6 GB | YES via `assets/` pattern (Wave-1 PR #69) | UNCLEAR | Keep as gitignored vendored CC0 assets directory; needs README documenting source (`fetch_polyhaven.py`, `fetch_ambientcg.py`). User decision: replace with `make fetch-assets` flow? |
| `vendor/{boat_attack,crest}/` | 933 MB | YES via `vendor/` pattern (Wave-1 PR #69) | UNCLEAR | Keep gitignored; both are `.zip` archives. User decision: should these be checked into LFS or fetched on demand by a script? |
...
### Wave 1 — Gitignore + untrack artifacts (zero risk)

Goal: lock down 31 GB of untracked PNG/blend output so accidental `git add .` is safe. Additive `.gitignore` edits plus `git rm --cached` on the 10 already-tracked binary artifacts (road/scatter renders + thumbnail) that should never have been committed.
