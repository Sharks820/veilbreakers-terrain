...
parent_proposal: .planning/proposals/2026-05-10_repo_reorg.md  # added by Wave-1 PR #69
---
...
The scope contamination has been memory-flagged since 2026-04-12 (internal session memory; no `project_procedural_meshes_scope.md` doc file exists in the repo).

## External surface area (importers — preserve EXACTLY)

| Importer | Line | Imports |
|---|---|---|
| `veilbreakers_terrain/handlers/environment.py` | :229 | `_detect_grid_dims`, `_detect_grid_dims_from_vertices` only (two helpers, not a bulk import) |
...
**Plus**: `pyright-strict-baseline.json` may gain new entries if file paths change (baseline currently has no `procedural_meshes` rows; verify after Phase 0 move). **Plus**: `docs/aaa-audit/GRADES_VERIFIED.csv` references procedural_meshes indirectly (via `_bridge_mesh.py` and `_scatter_engine.py` rows) but has no direct line-number anchors into `procedural_meshes.py` itself. **Plus**: docs that cite specific line ranges.
...
3. Add `procedural_meshes/__init__.py` containing:
   ...
   # Re-exports every public + private name from the legacy single-file module.
   # Domain extraction happens in Phases 1-N; the shim keeps external importers
   # green throughout.
   #
   # IMPORTANT: procedural_meshes.py defines a narrow __all__ (GENERATORS,
   # MeshSpec, _circle_points, _detect_grid_dims, _detect_grid_dims_from_vertices
   # only).  "from ._legacy import *" therefore does NOT re-export the ~290
   # generate_* functions.  Phase 0 must also expand __all__ in _legacy.py to
   # include every public generator name, OR list them all explicitly here.
   # The safest approach is to expand __all__ in _legacy.py first:
   #
   #   In _legacy.py, append all generate_* and other public names to __all__
   #   before the "from ._legacy import *" line below takes effect.
