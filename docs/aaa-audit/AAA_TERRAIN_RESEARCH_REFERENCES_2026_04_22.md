...
This reference list is used by `scripts/build_function_upgrade_path.py` to ground upgrade guidance in publicly verifiable sources from major engines, tools, and studio R&D.

## Core engine and tool references
...
- World Machine terrain and erosion docs:
...
## AAA studio and engine technical references

- Frostbite terrain system:
...
The function upgrade planner maps each callable into a domain track and attaches the relevant source links in `research_refs` for traceability during verification and implementation.
