...
| 0.5g | **T0.5-7** | **`test_geometric_quality.py:27` tautological delete** + re-import from `handlers.terrain_world_orchestration` | 0.5d | ZZ4-A4 P0 | **✓ LANDED** (PR #76 — yesterday) | `b946fa0b` |
...
| 0.5i | T0.5-9 | Split `test_full_terrain_pipeline.py` per-bundle + `run_pass` catch-and-tag | 1d | ZZ4-A6 R4 (cascade scoping) | **DEFERRED** — file doesn't exist; per-bundle test-file split already structurally present; run_pass catch-and-tag in place via FIX-D24-PR13 | — |
...
| **P2** | **T0.5-5b** | Fill 9 enum-only channels in `_CHANNEL_CANONICAL_UNITS` (tidal_zone_label, water_surface_mask, cave_candidate, flow_accumulation, erosion_amount, deposition_amount, curvature, bank_instability). Add `macro_color` to `Channel` enum. Bidirectional cross-registry walk. Remove stale skip-guard | Agent-3 coverage gap | 1 hr |
