...
2. **`127 random.Random sites`** → real value (Fix 4.11 ground-truth correction): **100 handlers + 79 tests = 179 production sites (per `RNG_SITES.txt` ground truth; was inflated by stale memory)**. Prior count "47 handlers + 11 tests = 58" was itself stale; the canonical ground-truth list under `.staging/RNG_SITES.txt` lists 179 sites. The "127" was 65 doc-prose mentions as RNG sites (Wave-3 V3 forensic, finding A.1 #4); the "58" was a partial scan. PR #18 effort upgraded L → XL per Fix 4.13 (was sized for 58).
