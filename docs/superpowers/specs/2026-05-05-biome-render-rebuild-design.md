...
2. **[AUTO-APPLIED — P3-Polish-2] After Block 1 PR #19** (Mei-2007 Taichi-CUDA hydraulic): bake 1
...
| 65 | chore(rng): delete 3 dead-RNG instantiations (Fix 4.10) | `terrain_features.py:2169` (`_ = rng`), `terrain_waterfalls.py:2280` (`_ = np.random.default_rng(...)`), `terrain_materials_v2.py:1046-1047` (`_ = _pass_rng`) | • All 3 dead-RNG instantiations removed (assigned to `_` and never used)<br>• No behavioral change<br>• Reduces RNG-site count noise in PR #18 ground-truth list | `pytest tests/test_dead_rng_cleanup.py`; `Grep "_ = rng\|_ = np.random.default_rng\|_ = _pass_rng" veilbreakers_terrain/` returns 0 | XS | none |
