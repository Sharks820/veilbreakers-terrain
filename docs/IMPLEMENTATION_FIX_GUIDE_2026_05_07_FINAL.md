...
- **D6-7:** B15-P0-01 affine rescale + B15-P0-02 4-biome crash + import-time biome-name invariant. **GATE D5** = topo-sort PR landed. ✅ DONE via PR #35.
- **D8-9:** PR #4 wire 8 orphan passes + 6 missing register_*_pass functions (snow_line shadow-write removed, stratigraphy/wind_erosion/coastline scheduled). ✅ DONE via PR #36.
- **D10-11:** PR #5a/#5b W-1 channel migration + B15-P0-05 caustics rename. ✅ DONE via PR #37.
- **D12-13:** B15-P0-08 hydraulic mass leak (75% boundary loss) + B15-P0-10 gradient axis swap + B15-P0-11 mean subtraction. ✅ DONE via PR #38 + #40 hotfix.
- **D14-15:** B15-P0-17 bedrock_height pre-integration + P0-18 hardness Y-axis vs Z-axis + P0-21 horizon_lod overrides; foam Beaufort cubic at `terrain_waterfalls.py:114-115`. **GATE D15** = bake-side P0 fixes landed; channel-graph audit green. ✅ DONE via PR #39.
...
## §21 PER-PHASE 2-VERIFIER PROTOCOL (added 2026-05-07 per user mandate)

**Rule:** at every Phase A/B/C/D/E boundary (i.e. GATE D5, D15, D25, D35, D45, D60), dispatch **2 parallel Opus verifier agents** with deliberately diverse angles. Both must converge GO before the next Phase begins.

**Why:** R1+R1.5+R2 burned multiple cycles on stale/contradictory audit data. Phase boundaries are the highest-leverage place to catch implementation drift, regressions, and gaps before they compound. Two angles per phase forces independent corroboration.

### §21.1 Verifier templates

| Verifier | Scope | Tools | Output |
|---|---|---|---|
| **V1 — Compliance & breadth** | Walk every Day-N step in §17 for the just-completed phase. For each task verify: (a) was it done? file:line evidence + git log; (b) does the GATE command at §8.5 pass; (c) spec PR cross-references resolved against current GitHub state; (d) no items deferred without explicit v1.1 marker. | Read, Grep, Glob, Bash | COMPLIANCE TABLE per Day-N + per-GATE; verdict GO/REWORK |
| **V2 — Quality & adversarial** | Run full pytest + pyright-strict + callable-census + license-manifest + biome-name invariant + determinism smoke (PYTHONHASHSEED=0 single-tile re-bake). Code-review the phase diff for silent failures, type hints, error handling, GPL/AGPL imports, naming, best practices. Adversarially try to break the changes. | Read, Grep, Bash | REGRESSION TABLE + BEST-PRACTICE FINDINGS + verdict GO/REWORK |

Both must return GO. Either REWORK = patches before next phase. Disagreement = third tiebreaker verifier OR surfaced to user.

### §21.2 Phase boundary checklist

Append this 4-step protocol to every GATE Dxx step in §17:

1. Commit + push the last task in the phase
2. Verify CI green on origin (or all merged PRs MERGED)
3. Dispatch V1 + V2 in parallel via Agent tool with the §21.1 templates
4. Reconcile both verdicts → only GO+GO advances; otherwise apply patches and re-verify

### §21.3 Anti-patterns

- **Sequential V1-then-V2** — loses parallelism + diversification; both must run simultaneously
- **Same prompt for both** — loses angle diversity; must be compliance vs adversarial
- **Skipping verification because "it's a small phase"** — every phase is 10-15 days of shippable terrain code
- **Letting V1 see V2's findings (or vice versa)** — each must verdict independently before reconciliation

### §21.4 10-verifier audit trail

The 60-day plan now requires 10 verifier runs (5 phases × 2 verifiers). Each pair appends a `## §13.X Phase-X Verifier Report` section to this doc with file:line evidence + GO/REWORK verdict. After Phase E V1+V2 + ship gate verdict, the doc accumulates a complete audit trail of every phase's compliance + quality posture.
