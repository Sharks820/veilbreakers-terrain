"""Mountain + Forest biome v1 — full integrated build.

4096m x 4096m mountain biome with:
  - Ridge silhouette terrain (steep relief, z up to ~320m)
  - 5-zone PBR shader (forest soil, alpine grass, scree, rock, snow)
  - Sun + Nishita sky + light volumetric haze
  - Forest belt: alpine pines + spruces + dwarf juniper at 5-130m elevation
  - Alpine grass + heather at 130-220m
  - Hero props: weathered boulders, fallen alpine logs
  - 8 named cameras for AAA multi-angle proof

Run::
    python scripts/mountain_build_v1_full.py
Then::
    python scripts/render_coastal_inline.py m1_mountain_forest \\
...
# ===== Heightfield: ridge silhouette ======================================
rng = np.random.default_rng(SEED)
axis = np.linspace(-half, half, GRID_N)
xx, yy = np.meshgrid(axis, axis)


def fbm_field(xx, yy, rng, oct, base_freq, persistence, lacunarity):
...
# Macro: 2-3 mountain peaks via large-scale features
macro = fbm_field(xx, yy, rng, oct=4, base_freq=0.0006, persistence=0.55, lacunarity=2.05)
# Ridge sharpening
ridge = 1.0 - np.abs(fbm_field(xx, yy, rng, oct=4, base_freq=0.0009, persistence=0.55, lacunarity=2.0))
ridge = ridge ** 1.6
# Combine
height_field = macro * 80.0 + ridge * 240.0

# Carve a valley running through tile
valley_y = -300.0
valley_band = np.exp(-((yy - valley_y) / 600.0) ** 2)
height_field = height_field * (1.0 - 0.55 * valley_band) - valley_band * 25.0

# Anchor 3 dominant peaks via Poisson-like positions
peaks = [
...
for px, py, ph, pr in peaks:
    ...
    height_field = np.maximum(height_field, height_field * (1.0 - fall) + ph * fall)

# Floor at -5m (small lakes possible)
z = np.maximum(height_field, -5.0)
print("VB_MTN_HEIGHT z_min={:.1f} z_max={:.1f} spread={:.1f}".format(
    float(z.min()), float(z.max()), float(np.percentile(z, 99) - np.percentile(z, 1))))


# ===== Slope, elevation, biome attrs ======================================
...
for o in list(bpy.data.objects):
    if o.name.startswith("VB_"):
        bpy.data.objects.remove(o, do_unlink=True)
for c in list(bpy.data.collections):
    if c.name.startswith("VB_"):
...
def add_attr(name, arr, domain="POINT", dtype="FLOAT"):
    a = mesh.attributes.new(name=name, type=dtype, domain=domain)
...
# ===== PBR shader (5 zones) ===============================================
m = bpy.data.materials.new("VB_MOUNTAIN_PBR")
m.use_nodes = True
nt = m.node_tree
...
# Per-zone procedural noise
soil_n = nt.nodes.new("ShaderNodeTexNoise"); soil_n.location = (-1200, 400)
soil_n.inputs["Scale"].default_value = 4.0; soil_n.inputs["Detail"].default_value = 8.0
nt.links.new(mapping.outputs[0], soil_n.inputs["Vector"])
grass_n = nt.nodes.new("ShaderNodeTexNoise"); grass_n.location = (-1200, 200)
grass_n.inputs["Scale"].default_value = 3.0; grass_n.inputs["Detail"].default_value = 6.0
nt.links.new(mapping.outputs[0], grass_n.inputs["Vector"])
scree_v = nt.nodes.new("ShaderNodeTexVoronoi"); scree_v.location = (-1200, 0)
scree_v.inputs["Scale"].default_value = 8.0
nt.links.new(mapping.outputs[0], scree_v.inputs["Vector"])
rock_v = nt.nodes.new("ShaderNodeTexVoronoi"); rock_v.location = (-1200, -200)
rock_v.inputs["Scale"].default_value = 4.0
nt.links.new(mapping.outputs[0], rock_v.inputs["Vector"])
snow_n = nt.nodes.new("ShaderNodeTexNoise"); snow_n.location = (-1200, -400)
snow_n.inputs["Scale"].default_value = 1.5; snow_n.inputs["Detail"].default_value = 4.0
...
def color_layer(base, dim, tint_in, x, y):
    ...
    nt.links.new(tint_in, cr.inputs[0])
    return cr


soil = color_layer((0.20, 0.18, 0.13), 0.65, soil_n.outputs["Fac"], -700, 400)
alpine_grass = color_layer((0.26, 0.32, 0.18), 0.62, grass_n.outputs["Fac"], -700, 200)
...
# Elevation-driven base layer mix
...
layer_soil   = mix(forest_mask.outputs[0], alpine_grass.outputs[0], soil.outputs[0], 0, 400)
layer_alpine = mix(alpine_mask.outputs[0], layer_soil.outputs[0], alpine_grass.outputs[0], 200, 300)
layer_scree  = mix(scree_mask.outputs[0], layer_alpine.outputs[0], scree.outputs[0], 400, 200)
layer_rock   = mix(slope_rock.outputs[0], layer_scree.outputs[0], rock.outputs[0], 600, 100)
layer_snow   = mix(snow_mask.outputs[0], layer_rock.outputs[0], snow.outputs[0], 800, 0)
nt.links.new(layer_snow.outputs[0], bsdf.inputs["Base Color"])

# Roughness: snow lower, rock higher
rough_calc = nt.nodes.new("ShaderNodeMapRange"); rough_calc.location = (800, -200)
rough_calc.inputs["From Min"].default_value = 0.0
rough_calc.inputs["From Max"].default_value = 1.0
rough_calc.inputs["To Min"].default_value = 0.92
rough_calc.inputs["To Max"].default_value = 0.55
nt.links.new(snow_mask.outputs[0], rough_calc.inputs["Value"])
nt.links.new(rough_calc.outputs[0], bsdf.inputs["Roughness"])
...
bump_node.inputs["Distance"].default_value = 0.18
...
def make_tree(name, trunk_h, trunk_r, branch_count, branch_len, leaf_radius,
              trunk_color, leaf_color, twist=0.0, lean=0.0, seed=7,
              shape="cone"):
    rng = np.random.default_rng(seed)
    verts, faces = [], []
    n_ring = 10; rings = 7
    ...
    for r in range(rings):
        for i in range(n_ring):
            faces.append((r*n_ring+i, r*n_ring+(i+1)%n_ring,
                          (r+1)*n_ring+(i+1)%n_ring, (r+1)*n_ring+i))
    top = len(verts); verts.append((lean, 0, trunk_h))
    for i in range(n_ring):
        faces.append((rings*n_ring+i, rings*n_ring+(i+1)%n_ring, top))

    branch_starts = []
    for b_idx in range(branch_count):
        ang = (2*math.pi*b_idx/branch_count) + rng.uniform(-0.25, 0.25)
        h_frac = rng.uniform(0.45, 0.95)
        ...
        d = (outward[0], outward[1], upward + rng.uniform(-0.15, 0.20))
        ...
        for r in range(5):
            ...
            for i in range(6):
                a = (2*math.pi*i)/6
                lx, ly = -d[1], d[0]
                verts.append((cx + rb*math.cos(a)*lx,
                              cy + rb*math.cos(a)*ly,
                              cz + rb*math.sin(a)))
        for r in range(4):
            for i in range(6):
                a = b_start + r*6 + i
                b_ = b_start + r*6 + (i+1)%6
                c_ = b_start + (r+1)*6 + (i+1)%6
                d_ = b_start + (r+1)*6 + i
    ...
    def add_blob(cx, cy, cz, radius):
        ...
        for j in range(1, n_v):
            ...
            for i in range(n_h):
                th = 2*math.pi*i/n_h
                verts.append((cx + radius*math.sin(phi)*math.cos(th),
        ...
        s0 = rs[0]
        for i in range(n_h):
            faces.append((s0+i, s0+(i+1)%n_h, top_idx))
        s0 = rs[-1]
        for i in range(n_h):
            faces.append((s0+(i+1)%n_h, s0+i, bot_idx))

    if shape == "cone":
        # Conical canopy: stacked cones via several blobs
        for k in range(4):
            t_ = k / 3
    ...
    mesh2.from_pydata(verts, [], faces); mesh2.update()
    o = bpy.data.objects.new(name, mesh2)
    veg_lib.objects.link(o)
    o.location = (10000, 10000, -1000)
...
def scatter(obj, elev_min, elev_max, slope_max, density, dist_min, x, y, label, align=True):
...
# Forest belt: pine 5-130m
pine_geo = scatter(pine,    elev_min=8.0,  elev_max=130.0, slope_max=38.0, density=0.0010, dist_min=14.0, x=-1300, y=400,  label="mtn_pine")
spruce_geo = scatter(spruce, elev_min=10.0, elev_max=120.0, slope_max=35.0, density=0.0007, dist_min=18.0, x=-1300, y=-700, label="mtn_spruce")
juniper_geo = scatter(juniper, elev_min=80.0, elev_max=200.0, slope_max=40.0, density=0.0030, dist_min=4.0, x=-1300, y=-1800, label="mtn_juniper")
heather_geo = scatter(heather, elev_min=120.0, elev_max=230.0, slope_max=42.0, density=0.0050, dist_min=2.5, x=-1300, y=-2900, label="mtn_heather")
grass_geo = scatter(alpine_g, elev_min=80.0, elev_max=240.0, slope_max=38.0, density=1.2,    dist_min=0.5, x=-1300, y=-4000, label="mtn_grass")

join = ns.new("GeometryNodeJoinGeometry"); join.location = (2800, 0)
ls.new(gi.outputs[0], join.inputs[0])
for src in (pine_geo, spruce_geo, juniper_geo, heather_geo, grass_geo):
    ls.new(src, join.inputs[0])
ls.new(join.outputs[0], go.inputs[0])
print("VB_MTN_SCATTER_DONE")
...
print("VB_MOUNTAIN_BUILT saved={}".format(out))
print("VB_MOUNTAIN_DONE")
'''
...
def main() -> int:
    ...
    try:
        with socket.create_connection(("127.0.0.1", 9876), timeout=10) as s:
            ...
            while time.time() < deadline:
                try:
                    ...
                    if not c: break
                    buf += c
                    if b"VB_MOUNTAIN_DONE" in buf: break
        ...
        return 3
    print(text[-2500:])
    return 0 if "VB_MOUNTAIN_DONE" in text else 2
