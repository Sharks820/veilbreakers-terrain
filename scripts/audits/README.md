...
| Script | Purpose |
|---|---|
| `audit_zero_asserts.py` | Scans test files (`test_*.py`) for test functions that contain no assertion, raise, or `pytest.raises`/`pytest.warns` call. Generates a JSON coverage report under `output/verification/`. |
