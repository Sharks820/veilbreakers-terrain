"""Mountain biome — built from the working Coastal V3e template.

Loads ``VB_Coastal_V3e_Props_4096m.blend``, removes Coastal-specific
content, replaces with Mountain-specific terrain + materials +
vegetation + cameras, keeps the proven lighting/world setup that
actually renders correctly.

Run::
    "C:/Program Files/Blender Foundation/Blender 4.5/blender.exe" \
        --background --python scripts/mountain_from_coastal_template.py
"""
...
COASTAL_TEMPLATE = REPO_ROOT / "output" / "visual_nodes" / "VB_Coastal_V3e_Props_4096m.blend"
OUT_BLEND = REPO_ROOT / "output" / "visual_nodes" / "VB_Mountain_Forest_v2_4096m.blend"
...
# Load template
bpy.ops.wm.open_mainfile(filepath=str(COASTAL_TEMPLATE))
print("LOADED " + str(COASTAL_TEMPLATE))


# Wipe Coastal-specific objects (keep world, sun, default scene)
remove_prefixes = (
    "VB_COASTAL", "VB_VEG_", "VB_PROP_", "VB_CORRECT_COASTAL",
)
...
peaks = [(1100., 500., 320., 700.), (-900., -1100., 280., 600.), (1500., -1500., 240., 550.)]
...
print("MTN_HEIGHT z_min={:.1f} z_max={:.1f}".format(float(z.min()), float(z.max())))
...
forest_mask = map_range(a_elev_n.outputs["Fac"], 5.0, 25.0, 1.0, 0.0, -400, 600)
alpine_mask = map_range(a_elev_n.outputs["Fac"], 100.0, 200.0, 0.0, 1.0, -400, 400)
scree_mask = map_range(a_elev_n.outputs["Fac"], 200.0, 260.0, 0.0, 1.0, -400, 200)
snow_mask = map_range(a_elev_n.outputs["Fac"], 260.0, 310.0, 0.0, 1.0, -400, 0)
slope_rock = map_range(a_slope_n.outputs["Fac"], 35.0, 60.0, 0.0, 1.0, -400, -200)
...
bump = nt.nodes.new("ShaderNodeBump"); bump.location = (200, -700)
bump.inputs["Strength"].default_value = 0.85; bump.inputs["Distance"].default_value = 0.18
...
pine    = make_tree("VB_MTN_PINE",    18.0, 0.42, 16, 1.8, 1.6, (0.16,0.10,0.07), (0.14,0.22,0.10), 0.1, 0.1, 11, "cone")
spruce  = make_tree("VB_MTN_SPRUCE",  22.0, 0.50, 18, 2.0, 2.0, (0.13,0.09,0.06), (0.10,0.18,0.09), 0.0, 0.05, 17, "cone")
juniper = make_tree("VB_MTN_JUNIPER",  2.5, 0.18, 6,  1.0, 0.9, (0.18,0.14,0.10), (0.20,0.28,0.13), 0.6, 0.3, 23, "round")
heather = make_tree("VB_MTN_HEATHER",  0.6, 0.10, 4,  0.4, 0.45, (0.30,0.22,0.17), (0.35,0.20,0.30), 0.4, 0.0, 29, "round")
alpine_g = make_grass_blade("VB_MTN_GRASS", 0.55, 0.04, (0.45, 0.55, 0.30))
...
a_el = ns.new("GeometryNodeInputNamedAttribute"); a_el.location = (-1500, 600); a_el.data_type = "FLOAT"
a_el.inputs[0].default_value = "vb_elev_m"
...
# ===== Cameras (re-create — Coastal cameras were removed) =================
def th(x_, y_):
...
bpy.context.scene.camera = bpy.data.objects["VB_MOUNTAIN_VALLEY"]

scene = bpy.context.scene
scene.render.engine = "BLENDER_EEVEE_NEXT"
scene.render.resolution_x = 1600; scene.render.resolution_y = 900

# ===== Sun + world (Coastal sun was removed by name-prefix cleanup) ======
bpy.ops.object.light_add(type="SUN", location=(0, -2000, 2400),
                         rotation=(math.radians(58), 0, math.radians(40)))
...
world = bpy.data.worlds.new("VB_MOUNTAIN_WORLD")
scene.world = world
...
manifest = {"unit_id": "m1_mountain_forest", "out_dir": str(OUT_DIR),
            "engine": scene.render.engine, "resolution": [scene.render.resolution_x, scene.render.resolution_y],
