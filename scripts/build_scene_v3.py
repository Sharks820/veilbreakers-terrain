...
# ---------------------------------------------------------------------------
# Tripo catalog — import and scatter GLB assets generated by Tripo Studio.
# Falls back silently when catalog.json is absent or all entries missing on
# disk so the build completes without Tripo auth.
# ---------------------------------------------------------------------------
def scatter_tripo_catalog(terrain_obj, hm, count: int = 40) -> int:
    import json
    ...
    if not cat_path.exists():
        log("tripo_catalog: catalog.json not found — skipping Tripo prop scatter")
        return 0

    try:
        catalog = json.loads(cat_path.read_text(encoding="utf-8"))
    except Exception as exc:
        log(f"tripo_catalog: parse error: {exc}")
        return 0

    # catalog["assets"] is a dict keyed by asset_id (written by ingest_tripo_asset.py)
    ...
    if not valid:
        log(f"tripo_catalog: {len(entries)} catalog entries, 0 GLBs on disk — skipping")
        return 0

    log(f"tripo_catalog: placing up to {count} instances from {len(valid)} GLBs")
    tripo_col = bpy.data.collections.new("VB_TripoProps")
    bpy.context.scene.collection.children.link(tripo_col)
    ...
    for asset in valid:
        ...
        try:
            ...
            log(f"tripo_catalog: import failed for {glb_path}: {exc}")
            continue
        ...
        while inst < instances_per and attempts < instances_per * 15:
            ...
            obj = bpy.data.objects.new(f"VB_Tripo_{placed:04d}", template.data)
            obj.location = (wx, wy, wz)
            scale = RNG.uniform(0.8, 1.4)
            obj.scale = (scale, scale, scale)
            obj.rotation_euler = (0.0, 0.0, RNG.uniform(0, math.tau))
            tripo_col.objects.link(obj)
            placed_xys.append((wx, wy))
            inst += 1
            placed += 1

    log(f"tripo_catalog: {placed} props placed from {len(valid)} catalog assets")
    return placed
...
def main() -> int:
    ...
    # ---- Tripo catalog props ----
    log("scattering Tripo catalog props (if available)...")
    n_tripo = 0
    try:
        n_tripo = scatter_tripo_catalog(terrain, hm, count=40)
    except Exception as exc:
        log_fail("tripo_catalog", exc)
