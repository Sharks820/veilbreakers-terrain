...
# ---------------------------------------------------------------------------
# Volcanic caldera generator (GAP-002)
# ---------------------------------------------------------------------------


def generate_volcanic_caldera(
    *,
    outer_radius: float = 50.0,
    inner_radius: float = 30.0,
    rim_height: float = 25.0,
    ...
    """Generate a volcanic caldera with outer slopes, rim, and inner crater.

    Real geometry: radial heightfield with volcanic profile, optional lava pool
    disc at the floor, and noise-perturbed rim for natural appearance.

    Returns dict with mesh (vertices, faces), materials, metadata.
    """
    rng = random.Random(seed)
    vertices: list[Vec3] = []
    faces: list[tuple[int, ...]] = []
    materials = ["volcanic_rock", "caldera_rim", "lava_pool"]
    mat_indices: list[int] = []

    inner_frac = inner_radius / outer_radius

    # Build ring-based mesh: rings from center out to outer_radius
    for ri in range(ring_segments + 1):
        r_frac = ri / ring_segments
        r = r_frac * outer_radius

        # Caldera profile: flat floor -> steep inner wall -> rim -> outer slope
        if r_frac < inner_frac * 0.6:
            z = floor_height
        elif r_frac < inner_frac:
            wall_t = (r_frac - inner_frac * 0.6) / (inner_frac * 0.4)
            wall_t = wall_t * wall_t * (3.0 - 2.0 * wall_t)  # smoothstep
            z = floor_height + (rim_height - floor_height) * wall_t
        elif r_frac < inner_frac + 0.05:
            rim_noise = _hash_noise(r * 0.1, ri * 0.3, seed) * 2.0
            z = rim_height + rim_noise
        else:
            slope_t = (r_frac - inner_frac - 0.05) / max(0.01, 1.0 - inner_frac - 0.05)
            z = rim_height * (1.0 - slope_t * slope_t)

        for ai in range(radial_segments):
            angle = 2.0 * math.pi * ai / radial_segments
            noise_r = _hash_noise(angle * 2.0, r * 0.05, seed + 1) * outer_radius * 0.03
            x = (r + noise_r) * math.cos(angle)
            y = (r + noise_r) * math.sin(angle)
            z_noise = _hash_noise(x * 0.1, y * 0.1, seed + 2) * 1.5
            vertices.append((x, y, z + z_noise))

    # Generate faces between rings
    for ri in range(ring_segments):
        for ai in range(radial_segments):
            ai_next = (ai + 1) % radial_segments
            v0 = ri * radial_segments + ai
            v1 = ri * radial_segments + ai_next
            v2 = (ri + 1) * radial_segments + ai_next
            v3 = (ri + 1) * radial_segments + ai
            faces.append((v0, v1, v2, v3))
            r_frac_face = ri / ring_segments
            if r_frac_face < inner_frac * 0.6:
                mat_indices.append(2)  # lava_pool
            elif abs(r_frac_face - inner_frac) < 0.1:
                mat_indices.append(1)  # caldera_rim
            else:
                mat_indices.append(0)  # volcanic_rock

    # Optional lava pool disc at crater floor
    lava_pool_verts: list[Vec3] = []
    if lava_pool:
        center_idx = len(vertices)
        vertices.append((0.0, 0.0, floor_height + 0.2))
        pool_r = inner_radius * 0.55
        pool_segs = min(radial_segments, 24)
        for pi in range(pool_segs):
            angle = 2.0 * math.pi * pi / pool_segs
            px = pool_r * math.cos(angle)
            py = pool_r * math.sin(angle)
            vertices.append((px, py, floor_height + 0.2))
            lava_pool_verts.append((px, py, floor_height + 0.2))
        for pi in range(pool_segs):
            pi_next = (pi + 1) % pool_segs
            faces.append((center_idx, center_idx + 1 + pi, center_idx + 1 + pi_next))
            mat_indices.append(2)
    ...
    return {
        ...
        "materials": materials,
        "material_indices": mat_indices,
        "lava_pool_verts": lava_pool_verts,
        "dimensions": {
...
# ---------------------------------------------------------------------------
# Columnar basalt generator (GAP-013)
# ---------------------------------------------------------------------------


def generate_columnar_basalt(
    *,
    num_columns: int = 24,
    ...
    """Generate columnar basalt — hexagonal column cluster.

    Real geometry: each column is a hexagonal prism with slightly varying
    height and radius. Columns pack roughly hexagonally within a cluster.
    ...
    vertices: list[Vec3] = []
    faces: list[tuple[int, ...]] = []
    materials = ["basalt_column", "basalt_top", "basalt_base"]
    mat_indices: list[int] = []

    column_positions: list[tuple[float, float, float]] = []
    for ci in range(num_columns):
        angle = 2.0 * math.pi * ci / max(1, num_columns)
        r = cluster_radius * math.sqrt(rng.random()) * 0.9
        cx = r * math.cos(angle) + rng.uniform(-column_radius, column_radius) * 0.5
        cy = r * math.sin(angle) + rng.uniform(-column_radius, column_radius) * 0.5
        col_h = rng.uniform(min_height, max_height)
        column_positions.append((cx, cy, col_h))

    for cx, cy, col_h in column_positions:
        base_idx = len(vertices)
        col_r = column_radius * rng.uniform(0.85, 1.15)

        # Bottom and top hex rings
        for z_val in [0.0, col_h]:
            for si in range(hex_segments):
                a = 2.0 * math.pi * si / hex_segments + rng.uniform(-0.05, 0.05)
                vertices.append((cx + col_r * math.cos(a), cy + col_r * math.sin(a), z_val))

        # Side faces
        for si in range(hex_segments):
            si_next = (si + 1) % hex_segments
            b0 = base_idx + si
            b1 = base_idx + si_next
            t0 = base_idx + hex_segments + si
            t1 = base_idx + hex_segments + si_next
            faces.append((b0, b1, t1, t0))
            mat_indices.append(0)

        # Top cap
        top_c = len(vertices)
        vertices.append((cx, cy, col_h))
        for si in range(hex_segments):
            faces.append((top_c, base_idx + hex_segments + si,
                          base_idx + hex_segments + (si + 1) % hex_segments))
            mat_indices.append(1)
        ...
        for si in range(hex_segments):
            faces.append((bot_c, base_idx + (si + 1) % hex_segments, base_idx + si))
            mat_indices.append(2)
...
# ---------------------------------------------------------------------------
# Sediment deposition: alluvial fan (GAP-008)
# ---------------------------------------------------------------------------


def generate_alluvial_fan(
    *,
    apex_height: float = 15.0,
    fan_radius: float = 40.0,
    spread_angle: float = 120.0,
    radial_segments: int = 20,
    angular_segments: int = 32,
    seed: int = 42,
) -> dict:
    """Generate an alluvial fan / delta deposition feature.

    Real geometry: fan-shaped heightfield from apex spreading outward
    with decreasing height and braided sediment channels.

    Returns dict with mesh, materials, metadata.
    """
    rng = random.Random(seed)
    vertices: list[Vec3] = []
    faces: list[tuple[int, ...]] = []
    materials = ["coarse_sediment", "fine_sediment", "channel_gravel"]
    mat_indices: list[int] = []

    spread_rad = math.radians(spread_angle)
    start_angle = -spread_rad / 2.0
    end_angle = spread_rad / 2.0

    num_channels = max(3, angular_segments // 6)
    channel_angles = [
        start_angle + (end_angle - start_angle) * (i + rng.uniform(0.2, 0.8)) / num_channels
        for i in range(num_channels)
    ]

    for ri in range(radial_segments + 1):
        r_frac = ri / radial_segments
        r = r_frac * fan_radius

        for ai in range(angular_segments + 1):
            a_frac = ai / angular_segments
            angle = start_angle + a_frac * (end_angle - start_angle)

            z = apex_height * (1.0 - r_frac) ** 1.5

            channel_depth = 0.0
            for ch_angle in channel_angles:
                angular_dist = abs(angle - ch_angle)
                ch_width = 0.06 + 0.04 * r_frac
                if angular_dist < ch_width:
                    depth_factor = 1.0 - angular_dist / ch_width
                    channel_depth = max(channel_depth, 1.5 * depth_factor * (0.5 + 0.5 * r_frac))

            x = r * math.cos(angle)
            y = r * math.sin(angle)
            z += _hash_noise(x * 0.05, y * 0.05, seed) * 1.0 * r_frac
            z -= channel_depth
...
def generate_cirque(
    *,
    radius: float = 40.0,
    headwall_height: float = 30.0,
    floor_depth: float = -5.0,
    radial_segments: int = 24,
    ring_segments: int = 12,
    seed: int = 42,
    has_tarn: bool = True,
) -> dict:
    """Generate a glacial cirque — amphitheater-shaped basin.

    Real geometry: half-bowl with steep headwall, curved floor, and optional
    tarn (glacial lake) at the base. Armchair-shaped profile.

    Returns dict with mesh, materials, metadata.
    """
    vertices: list[Vec3] = []
    faces: list[tuple[int, ...]] = []
    materials = ["headwall_rock", "cirque_floor", "tarn_water", "moraine_debris"]
    mat_indices: list[int] = []
    ...
    for ri in range(ring_segments + 1):
        ...
        for ai in range(radial_segments + 1):
            a_frac = ai / radial_segments
            angle = math.pi * a_frac

            x = r * math.cos(angle)
            y = r * math.sin(angle)

            if r_frac < 0.3:
                z = floor_depth + r_frac * 2.0
            elif r_frac < 0.8:
                t = (r_frac - 0.3) / 0.5
                t_smooth = t * t * (3.0 - 2.0 * t)
                z = floor_depth + 2.0 * 0.3 + t_smooth * (headwall_height * 0.6 - floor_depth)
            else:
                t = (r_frac - 0.8) / 0.2
                z = headwall_height * 0.6 + t * headwall_height * 0.4

            back_factor = math.sin(angle)
            z *= 0.5 + 0.5 * back_factor
            z += _hash_noise(x * 0.08, y * 0.08, seed) * 2.0

            vertices.append((x, y, z))

    for ri in range(ring_segments):
        for ai in range(radial_segments):
            row_w = radial_segments + 1
            v0 = ri * row_w + ai
            v1 = ri * row_w + ai + 1
            v2 = (ri + 1) * row_w + ai + 1
            v3 = (ri + 1) * row_w + ai
            ...
            r_frac = ri / ring_segments
            if r_frac < 0.3:
                mat_indices.append(2 if has_tarn else 1)
            elif r_frac > 0.7:
                mat_indices.append(0)
            else:
                mat_indices.append(3)
    ...
    return {
        ...
        "dimensions": {
            "radius": radius,
            "headwall_height": headwall_height,
...
def generate_cliff_stratigraphy(
    *,
    width: float = 30.0,
    height: float = 25.0,
    depth: float = 5.0,
    num_layers: int = 8,
    horizontal_segments: int = 24,
    seed: int = 42,
) -> dict:
    """Generate a cliff face with visible rock strata layers.

    Real geometry: vertical cliff surface with horizontal layer offsets
    creating visible strata. Harder layers protrude, softer layers recede.
    ...
    vertices: list[Vec3] = []
    faces: list[tuple[int, ...]] = []
    materials = ["sandstone", "limestone", "shale", "conglomerate"]
    mat_indices: list[int] = []

    layer_defs: list[dict] = []
    z_cursor = 0.0
    for li in range(num_layers):
        layer_h = (height / num_layers) * rng.uniform(0.6, 1.4)
        hardness = rng.uniform(0.2, 1.0)
        mat_idx = rng.randint(0, len(materials) - 1)
        layer_defs.append({
            "z_start": z_cursor,
            "z_end": z_cursor + layer_h,
            "hardness": hardness,
            "material": mat_idx,
        })
        z_cursor += layer_h

    vert_per_row = horizontal_segments + 1
    for layer in layer_defs:
        z_lo = layer["z_start"]
        z_hi = layer["z_end"]
        hardness = layer["hardness"]

        # Front face: two rows (bottom, top) at varying depth
        for z_val in [z_lo, z_hi]:
            for xi in range(vert_per_row):
                x_frac = xi / horizontal_segments
                x = x_frac * width - width / 2.0
                d = depth * (0.3 + 0.7 * hardness)
                d += _hash_noise(x * 0.15, z_val * 0.15, seed + layer["material"]) * 0.5
                d = max(0.1, d)
                vertices.append((x, -d, z_val))

        # Back face: two rows flush at y=0
        for z_val in [z_lo, z_hi]:
            for xi in range(vert_per_row):
                x_frac = xi / horizontal_segments
                x = x_frac * width - width / 2.0
                vertices.append((x, 0.0, z_val))

    rows_per_layer = 4
    for li, layer in enumerate(layer_defs):
        base = li * rows_per_layer * vert_per_row
        front_lo = base
        front_hi = base + vert_per_row
        back_lo = base + 2 * vert_per_row
        back_hi = base + 3 * vert_per_row

        for xi in range(horizontal_segments):
            faces.append((front_lo + xi, front_lo + xi + 1, front_hi + xi + 1, front_hi + xi))
            mat_indices.append(layer["material"])
            faces.append((back_lo + xi, back_hi + xi, back_hi + xi + 1, back_lo + xi + 1))
            mat_indices.append(layer["material"])
            faces.append((front_hi + xi, front_hi + xi + 1, back_hi + xi + 1, back_hi + xi))
            mat_indices.append(layer["material"])
            faces.append((front_lo + xi, back_lo + xi, back_lo + xi + 1, front_lo + xi + 1))
            mat_indices.append(layer["material"])
    ...
    return {
        ...
        "dimensions": {"width": width, "height": z_cursor, "depth": depth},
        "vertex_count": len(vertices),
        "face_count": len(faces),
    }
...
def generate_debris_cone(
    *,
    apex_height: float = 20.0,
    base_radius: float = 25.0,
    radial_segments: int = 32,
    ring_segments: int = 10,
    rock_count: int = 40,
    seed: int = 42,
) -> dict:
    """Generate a debris/talus cone at the base of a cliff.

    Real geometry: conical mound of accumulated rockfall with scattered
    boulder placement data on the surface.
    ...
    vertices: list[Vec3] = []
    faces: list[tuple[int, ...]] = []
    materials = ["talus_rock", "fine_debris", "boulder"]
    mat_indices: list[int] = []

    for ri in range(ring_segments + 1):
        r_frac = ri / ring_segments
        r = r_frac * base_radius
        z = apex_height * (1.0 - r_frac ** 0.7)  # concave-up angle-of-repose

        for ai in range(radial_segments):
            angle = 2.0 * math.pi * ai / radial_segments
            noise_r = _hash_noise(angle * 2.0, r * 0.1, seed) * base_radius * 0.05
            noise_z = _hash_noise(angle * 3.0, r * 0.15, seed + 1) * 1.5
            vx = (r + noise_r) * math.cos(angle)
            vy = (r + noise_r) * math.sin(angle)
            vertices.append((vx, vy, z + noise_z))
    ...
    for ri in range(ring_segments):
        for ai in range(radial_segments):
            ...
            v3 = (ri + 1) * radial_segments + ai
            faces.append((v0, v1, v2, v3))
            mat_indices.append(0 if ri / ring_segments < 0.6 else 1)

    boulders: list[dict] = []
    for _ in range(rock_count):
        b_angle = rng.uniform(0, 2.0 * math.pi)
        b_r = base_radius * math.sqrt(rng.uniform(0.1, 1.0))
        b_z = apex_height * (1.0 - (b_r / base_radius) ** 0.7)
        boulders.append({
            "position": (b_r * math.cos(b_angle), b_r * math.sin(b_angle), b_z),
