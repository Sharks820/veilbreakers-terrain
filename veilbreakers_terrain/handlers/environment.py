...
def _create_terrain_mesh_from_heightmap(
    *,
    ...
    # T0-3.5: try/finally guarantees bm.free() runs even on exception.
    bm = bmesh.new()
    try:
    ...
    if cliff_overlays_enabled:
        ...
        for i, cp in enumerate(cliff_placements):
            ...
            # T0-3.5: try/finally guarantees cliff_bm.free() on every exit path.
            cliff_bm = bmesh.new()
            try:
    ...
    if terrain_name and _HAS_BPY:
        ...
        if terrain_obj is not None and terrain_obj.type == "MESH":
            try:
                ...
                # T0-3.5: try/finally guarantees t_bm.free() on every exit path.
                t_bm = bmesh.new()
                try:
...
def handle_generate_road(params: dict[str, Any]) -> dict[str, Any]:
    ...
    # `if _road_strict: raise` re-raise inside the inner road-solver try block.
    bm = bmesh.new()
    try:
        bm.from_mesh(mesh)
        bm.verts.ensure_lookup_table()
