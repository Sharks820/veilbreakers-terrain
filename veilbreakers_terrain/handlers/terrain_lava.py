...
def _compute_proximity(
    ...
    back to an exact lava-cell mask so the channel is never None.

    The result is inverted and normalised to [0, 1]: 1 = at lava edge, 0 = far.
    """
    rows, cols = surface_mask.shape
    ...
    try:
        ...
        # the complement.
        dist = np.asarray(distance_transform_edt(1 - binary), dtype=np.float64)
        max_dist = float(dist.max())
        if max_dist > 0.0:
            prox = 1.0 - np.clip(dist / max_dist, 0.0, 1.0)
        else:
            prox = np.ones((rows, cols), dtype=np.float64)
    except ImportError:
...
def pass_lava_simulation(
    ...
    if not _biome_str:
        _hints = intent.composition_hints
    ...
    _hints_map = intent.composition_hints
    ...
    stack.set("lava_depth", lava_depth, "pass_lava_simulation")
    stack.set("lava_prox", lava_prox, "pass_lava_simulation")
    stack.set("lava_surface_mask", surface_mask, "pass_lava_simulation")
