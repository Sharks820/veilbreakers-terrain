...
def phacelle_noise(
    ...
    # Keep trig arguments bounded for far-world tiles.  The stripe phase is
    # periodic over one projected cell, so wrapping before sin/cos preserves
    # the value while avoiding precision loss at large world origins.
    phase = np.remainder(proj, 1.0) * (2.0 * np.pi)
