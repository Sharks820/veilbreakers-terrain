...
class TestMultiMaterialThermalErosion:
    """Multi-material thermal erosion: sediment depleted before bedrock."""

    def test_sediment_depleted_before_bedrock(self):
        """Erosion removes sediment first, bedrock only when sediment exhausted."""
        from blender_addon.handlers._terrain_erosion import (
            apply_multi_material_thermal_erosion,
        )

        # Peak with thick sediment — erosion should remove sediment only
        hmap = np.zeros((9, 9), dtype=np.float64)
        hmap[4, 4] = 2.0
        sediment = np.full_like(hmap, 1.0)  # 1m of sediment everywhere
        bedrock = hmap - sediment  # bedrock is hmap - sediment
        rock_hardness = np.full_like(hmap, 0.8)  # hard rock

        result = apply_multi_material_thermal_erosion(
            hmap, sediment_height=sediment, bedrock_height=bedrock,
            rock_hardness=rock_hardness, iterations=5, talus_angle=30.0,
        )

        # Sediment at peak should be reduced (some material moved)
        assert result.sediment_height[4, 4] < sediment[4, 4], (
            "Sediment at peak should be depleted by thermal erosion"
        )
        # Total height should be modified
        assert not np.array_equal(result.height, hmap)

    def test_deposition_adds_to_sediment(self):
        """Material deposited downslope goes into sediment buffer."""
        from blender_addon.handlers._terrain_erosion import (
            apply_multi_material_thermal_erosion,
        )

        hmap = np.zeros((9, 9), dtype=np.float64)
        hmap[4, 4] = 3.0  # Steep peak
        sediment = np.full_like(hmap, 0.5)
        bedrock = hmap - sediment
        rock_hardness = np.full_like(hmap, 0.5)

        result = apply_multi_material_thermal_erosion(
            hmap, sediment_height=sediment, bedrock_height=bedrock,
            rock_hardness=rock_hardness, iterations=10, talus_angle=25.0,
        )

        # Neighbors of peak should have MORE sediment (deposition)
        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            assert result.sediment_height[4 + dr, 4 + dc] >= sediment[4 + dr, 4 + dc], (
                f"Sediment at ({4+dr},{4+dc}) should increase from deposition"
            )

    def test_rock_hardness_affects_erosion_rate(self):
        """Harder rock erodes slower than softer rock."""
        from blender_addon.handlers._terrain_erosion import (
            apply_multi_material_thermal_erosion,
        )
        ...
        hmap = np.zeros((9, 9), dtype=np.float64)
        hmap[4, 4] = 2.0
        sediment = np.zeros_like(hmap)  # No sediment — pure bedrock
        bedrock = hmap.copy()

        # Hard rock
        hard = np.full_like(hmap, 0.95)
        result_hard = apply_multi_material_thermal_erosion(
            hmap.copy(), sediment_height=sediment.copy(),
            bedrock_height=bedrock.copy(), rock_hardness=hard,
            iterations=10, talus_angle=30.0,
        )

        # Soft rock
        soft = np.full_like(hmap, 0.1)
        result_soft = apply_multi_material_thermal_erosion(
            hmap.copy(), sediment_height=sediment.copy(),
            bedrock_height=bedrock.copy(), rock_hardness=soft,
            iterations=10, talus_angle=30.0,
        )

        # Soft rock peak should erode MORE (lower peak height)
        assert result_soft.height[4, 4] < result_hard.height[4, 4], (
            f"Soft rock ({result_soft.height[4,4]:.4f}) should erode more than "
            f"hard rock ({result_hard.height[4,4]:.4f})"
        )
