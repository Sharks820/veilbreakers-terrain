...
def test_staged_plan_preserves_aaa_quality_but_serializes_heavy_work() -> None:
    ...
    assert plan["execution_policy"]["local_hunyuan_allowed"] is False
...
def test_external_asset_request_plan_is_remote_hunyuan_cache_first_contract() -> None:
    recipe = build_reference_scene_recipe(
    ...
    for request in requests:
        ...
        assert request.extra["provider_id"] == "hunyuan3d2"
        assert request.extra["remote_only"] is True
        assert request.extra["local_hunyuan_allowed"] is False
