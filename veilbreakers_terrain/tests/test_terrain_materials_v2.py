...
# ---------------------------------------------------------------------------
# DAG-to-Blender bridge tests
# ---------------------------------------------------------------------------


def test_dag_weights_to_rgba_shape():
    from blender_addon.handlers.terrain_materials_v2 import (
        compute_slope_material_weights,
        dag_weights_to_rgba,
        default_dark_fantasy_rules,
    ...
    state = _build_state()
    weights = compute_slope_material_weights(state.mask_stack)
    rgba = dag_weights_to_rgba(weights)
    H, W = state.mask_stack.height.shape
    assert rgba.shape == (H, W, 4)
    assert rgba.dtype == np.float32


def test_dag_weights_to_rgba_normalized():
    from blender_addon.handlers.terrain_materials_v2 import (
        compute_slope_material_weights,
        dag_weights_to_rgba,
    )
    ...
    weights = compute_slope_material_weights(state.mask_stack)
    rgba = dag_weights_to_rgba(weights)
    sums = rgba.sum(axis=2)
    assert np.allclose(sums, 1.0, atol=1e-5), f"RGBA sums not 1.0: min={sums.min()}, max={sums.max()}"


def test_dag_weights_to_rgba_maps_channels():
    from blender_addon.handlers.terrain_materials_v2 import (
        dag_weights_to_rgba,
        default_dark_fantasy_rules,
    )
    ...
    rules = default_dark_fantasy_rules()
    # Create synthetic weights: all ground
    weights = np.zeros((4, 4, 5), dtype=np.float32)
    weights[:, :, 0] = 1.0  # ground channel
    rgba = dag_weights_to_rgba(weights, rules)
    # Ground maps to R channel
    assert np.allclose(rgba[:, :, 0], 1.0)
    assert np.allclose(rgba[:, :, 1], 0.0)
    assert np.allclose(rgba[:, :, 2], 0.0)
    assert np.allclose(rgba[:, :, 3], 0.0)


def test_dag_weights_to_rgba_cliff_maps_to_blue():
    from blender_addon.handlers.terrain_materials_v2 import (
        dag_weights_to_rgba,
        default_dark_fantasy_rules,
    )
    ...
    rules = default_dark_fantasy_rules()
    weights = np.zeros((4, 4, 5), dtype=np.float32)
    weights[:, :, 1] = 1.0  # cliff channel (index 1 in default rules)
    rgba = dag_weights_to_rgba(weights, rules)
    # Cliff maps to B channel
    assert np.allclose(rgba[:, :, 2], 1.0)


def test_dag_weights_to_rgba_snow_maps_to_alpha():
    from blender_addon.handlers.terrain_materials_v2 import (
        dag_weights_to_rgba,
        default_dark_fantasy_rules,
    )
    ...
    rules = default_dark_fantasy_rules()
    weights = np.zeros((4, 4, 5), dtype=np.float32)
    weights[:, :, 4] = 1.0  # snow channel (index 4)
    rgba = dag_weights_to_rgba(weights, rules)
    # Snow maps to A channel (special)
    assert np.allclose(rgba[:, :, 3], 1.0)


def test_dag_weights_integrates_with_pass_materials():
    """End-to-end: pass_materials -> dag_weights_to_rgba produces valid RGBA."""
    from blender_addon.handlers.terrain_materials_v2 import (
        dag_weights_to_rgba,
        register_bundle_b_material_passes,
    )
    ...
    splatmap = state.mask_stack.get("splatmap_weights_layer")
    assert splatmap is not None
    rgba = dag_weights_to_rgba(np.asarray(splatmap))
