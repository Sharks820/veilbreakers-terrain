...
class Hunyuan3D2Provider(ExternalAssetProvider):
    ...
    def is_available(self) -> bool:
        """Return True if the backend is reachable and gradio_client is installed.

        Does a lightweight Client() connection attempt with a 10-second timeout.
        Returns False on any import/network error so callers can fall back to
        procedural geometry without raising.
        """
        try:
            client = self._get_gradio_client()
            _ = client  # connection succeeded
            return True
        except Exception:
            return False
    ...
    def _prune_finished_jobs_locked(self) -> None:
        """Drop stale finished jobs. Caller must hold ``_jobs_lock``."""
        ...
        for job_id, (_thread, holder) in self._jobs.items():
            ...
            if status not in (JobStatus.COMPLETED, JobStatus.FAILED) or finished_at is None:
                continue
